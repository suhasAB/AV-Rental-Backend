...
export const getInRideBalance = (req, res) => {
    try{
        const userId = req.query.userId;
       
        console.log(userId);
        const inRideBalance = `select sum(charges) as balance from ride 
                where customerId = ?;`;
        con.query(inRideBalance, [userId], (err, result)=>{
            if(err){
                console.log(err);
                sendInternalServerError(res);
            }
            else{
                console.log('Rides balance', result);
                sendCustomSuccess(res, result); 
            }
        })
    }
